...
O jogo ocorre da seguinte forma. Inicialmente, um tabuleiro com
* dimensões N × N é dado contendo apenas 0’s. Depois disso, Q operações são propostas, podendo ser de 4 tipos:
...
public class TabuleiroSecreto {
        ...
        private static void imprimirFrequenteColunaX(int col, int[][] matriz) {
            ...
            for (Map.Entry<Integer, Integer> integerIntegerEntry : map.entrySet()) {
                if (!seen || (integerIntegerEntry.getValue() > best.getValue() ? 1 : -1) > 0) {
                    seen = true;
                    best = integerIntegerEntry;
                }
            }
            var key = (seen ? Optional.of(best) : Optional.<Map.Entry<Integer, Integer>>empty()).get().getKey();
